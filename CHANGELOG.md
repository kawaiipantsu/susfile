...
- README rewrite with real TUI screenshots (SVG, generated by
  `assets/screenshot.sh` via `assets/framegen` + `charmbracelet/freeze` — no
  pseudo-terminal needed), install paths (`install.sh`, `.deb`, source),
  the class legend and the `E`/`Z` caveat. `docs/releases.md`; `tui.Frame`
  helper for screenshots and golden tests.
