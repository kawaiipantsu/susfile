#!/bin/sh
# susfile installer. Detects your Linux arch, downloads the matching release
# archive, verifies it against the release SHA256SUMS, and installs the binary.
#
#   curl -fsSL https://raw.githubusercontent.com/kawaiipantsu/susfile/main/install.sh | sh
#
# Environment:
#   SUSFILE_VERSION   version to install (default: latest release)
#   SUSFILE_INSTALL   install directory (default: ~/.local/bin, or /usr/local/bin if writable)
set -eu

REPO="kawaiipantsu/susfile"
BIN="susfile"
...
install -m 0755 "$TMP/${BIN}_${VER}_linux_${ARCH}/$BIN" "$DEST/$BIN"
