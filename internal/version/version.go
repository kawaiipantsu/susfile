...
//
//	-X github.com/kawaiipantsu/susfile/internal/version.Version=...
var (
...
// String returns a multi-line, human-readable build stamp.
func String() string {
	dirty := ""
	if Dirty == "true" {
		dirty = " (dirty)"
	...
	return fmt.Sprintf(
		"susfile v%s%s\ncommit:  %s\nbuilt:   %s\ngo:      %s\nos/arch: %s/%s",
		Version, dirty, Commit, Date, runtime.Version(), runtime.GOOS, runtime.GOARCH,
	)
}

// Short returns a one-line version string.
func Short() string {
	return fmt.Sprintf("susfile v%s (%s)", Version, Commit)
}
